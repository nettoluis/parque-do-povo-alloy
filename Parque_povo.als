...
-- Garantir que todo ingresso tem um único setor restrito associado
assert ingressoUnicoSetorRestrito{
    	Ingresso in SetorRestrito.ingressos and all disj s1, s2 : SetorRestrito | no (s1.ingressos & s2.ingressos)
}
...
--Garantir que todo setor está associado a exatamente um dia
assert setorAssociadoDia{
	all s: Setor | one d: Dia | s = d.camarote or s = d.frontstage or s = d.pista
}
--Garantir que se uma pessoa está em um setor restrito, ela também tem acesso a pista daquele dia
assert acessoAPistaParaSetoresRestritos{
	all d: Dia, p: Pessoa | (p in d.camarote.ingressos.dono or p in d.frontstage.ingressos.dono) implies p in d.pista.acessos
}
...
check setorAssociadoDia for 15 but exactly 5 Dia, 5 Pessoa
check acessoAPistaParaSetoresRestritos for 15 but exactly 5 Dia, 5 Pessoa
